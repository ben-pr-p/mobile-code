...
import type { DurableStream, DurableStreamOptions, StreamResponse } from '@durable-streams/client';
...
export type StateSchema<T extends Record<string, CollectionDefinition>> = {
  [K in keyof T]: CollectionWithHelpers<T[K] extends CollectionDefinition<infer U> ? U : unknown>;
};
...
export interface CreateStreamDBOptions<
  ...
  TActions extends Record<string, ActionDefinition<any>> = Record<string, never>,
> {
 ...
 */
const RESERVED_COLLECTION_NAMES = new Set([`collections`, `preload`, `close`, `utils`, `actions`]);
...
function createCollectionHelpers<T>(
  ...
  return {
    insert: ({ key, value, headers }): ChangeEvent<T> => {
      ...
      const finalKey = key ?? (derived != null && derived !== `` ? String(derived) : undefined);
    ...
    delete: ({ key, oldValue, headers }): ChangeEvent<T> => {
      ...
      const finalKey = key ?? (oldValue ? String((oldValue as any)[primaryKey]) : undefined);
      if (!finalKey) {
        throw new Error(
 ...
 */
export function createStateSchema<T extends Record<string, CollectionDefinition>>(
  collections: T
): StateSchema<T> {
...
>(
...
): TActions extends Record<string, never> ? StreamDB<TDef> : StreamDBWithActions<TDef, TActions> {
  ...
  const dbMethods: StreamDBMethods = {
    ...
    utils: {
      awaitTxId: (txid: string, timeout?: number) => dispatcher.awaitTxId(txid, timeout),
    },
  };
  ...
  if (actionsFactory) {
    ...
    const wrappedActions: Record<string, ReturnType<typeof createOptimisticAction>> = {};
 ...
 */
export function createCapturingSyncConfig(): {
 ...
 */
export function appendStreamToDb(
  ...
  for (const name of collectionNames) {
    ...
    if (!entry) {
      throw new Error(
        `[appendStreamToDb] Collection "${name}" not found. ` +
